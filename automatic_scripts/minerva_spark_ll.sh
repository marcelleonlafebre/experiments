...
--instance-groups InstanceGroupType=MASTER,Name=Master,InstanceCount=1,InstanceType=m4.large \
InstanceGroupType=CORE,Name=Core,InstanceCount=1,InstanceType=m4.large \
InstanceGroupType=TASK,Name=Task-1,InstanceCount=1,InstanceType=m4.large \
InstanceGroupType=TASK,Name=Task-2,InstanceCount=1,InstanceType=m4.large \
InstanceGroupType=TASK,Name=Task-3,InstanceCount=1,InstanceType=m4.large \
InstanceGroupType=TASK,Name=Task-4,InstanceCount=1,InstanceType=m4.large \
InstanceGroupType=TASK,Name=Task-5,InstanceCount=1,InstanceType=m4.large \
InstanceGroupType=TASK,Name=Task-6,InstanceCount=1,InstanceType=m4.large \
InstanceGroupType=TASK,Name=Task-7,InstanceCount=1,InstanceType=m4.large \
--log-uri s3://bigdatamell1/log_spark --ec2-attributes KeyName=emr-keypair,AdditionalMasterSecurityGroups=$MY_SG,SubnetId=$SUBNET_ID --no-auto-terminate --profile usrmll | jq -r ".ClusterId")
echo $CLUSTER_ID
