...
CLUSTER_ID=$(aws emr create-cluster --name autoClusterHive --applications Name=Hadoop Name=Hive --configurations file://conf_emr_hive.json --release-label emr-6.13.0 --use-default-roles \
  ...
  InstanceGroupType=TASK,Name=Task-21,InstanceCount=1,InstanceType=m5.xlarge \
  InstanceGroupType=TASK,Name=Task-22,InstanceCount=1,InstanceType=m5.xlarge \
  InstanceGroupType=TASK,Name=Task-23,InstanceCount=1,InstanceType=m5.xlarge \
  InstanceGroupType=TASK,Name=Task-24,InstanceCount=1,InstanceType=m5.xlarge \
  InstanceGroupType=TASK,Name=Task-25,InstanceCount=1,InstanceType=m5.xlarge \
...
ssh -t ec2-user@$MASTER_URL -i emr-keypair.pem "sudo su -c 'mv /home/ec2-user/tpcds_query.sql /root/tpcds_query.sql;cd /root;hive -f tpcds_query.sql>tpcds_query.out;aws s3 cp tpcds_query.out s3://bigdatamell/log_hive/$query.out;exit;exit'"
